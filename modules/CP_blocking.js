...
function blockingPlayer(code, options) {
    ...
    if (modules.blocking.status && modules.blocking.data[modules.blocking.data.display]) {
        ...
        if (modules.blocking.data.display == 'share') {
            ...
            code.player = '<div id="blocking" style="position:absolute;background:#000 url(/themes/default/public/images/theme/dark/player.png) 100% 100% no-repeat;background-size:100% 100%;z-index:10000;top:0;left:0;width:100%;height:100%;color:#fff;text-align:center"><div id="blockingMessage" style="margin:30px auto 0;width:70%">' + message + '</div><div id="blockingSkip" style="color:white; margin:30px auto 0;width:70%; display: none;cursor: pointer;">' + skipText + '</div><div id="blockingCode" style="margin:30px auto">' + block.code + '</div></div><script>window.addEventListener("load",function(){var e=document.getElementById("blockingTimer"),n=' + block.time + ';var si=setInterval(function(){if(e.innerHTML=""+n,n=parseInt(n),n--,0>n){' + skipCode + 'clearInterval(si)}},1e3);var s=document.getElementById("blockingSkip");s.addEventListener("click",function(){var t=document.getElementById("blocking");t.parentElement.removeChild(t);})});</script>' + code.player;
