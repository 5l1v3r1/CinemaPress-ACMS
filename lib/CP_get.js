            ...
            if (query.hasOwnProperty(attribute)) {
                ...
                if (attribute == 'type') {
                    if (search == config.urls.types.serial.toLowerCase()) {
                        ...
                        return callback('Данного типа фильмов нет на сайте. Возможно Вы ошиблись в URL или это внутренняя ошибка сайта, о которой администратор уже знает и предпринимает действия для её устранения.');
                    }
                }
                else {
