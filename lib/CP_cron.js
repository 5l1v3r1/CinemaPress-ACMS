    ...
    async.eachOfLimit(keys, 1, function (arr, key, callback) {
        ...
        if (key === 'moonwalk_movies' && modules.player.data.moonwalk.token) {
            async.eachOf(arr, function (url, id, callback) {
                ...
                request({url: url, method: 'GET', timeout: 1000}, function (error, response, movies) {
                    if (error) {
                    ...
                    if (response.statusCode === 200 && movies && movies.updates && movies.updates.length) {
                        movies.updates.forEach(function (movie) {
                            if (movie && movie.kinopoisk_id && parseInt(movie.kinopoisk_id)) {
                                ...
                                if (movie.kinopoisk_id && !(ids.indexOf(movie.kinopoisk_id)+1)) {
                                    ids.push(movie.kinopoisk_id);
                                    mvs.push({
                                        "kinopoisk_id": movie.kinopoisk_id,
                                        "added_at": movie.added_at,
                                        "quality": movie.source_type && modules.content.data.auto[key].quality
                                            ? movie.source_type
                                            : '',
                                        "translate": movie.translator && modules.content.data.auto[key].translate
                                            ? movie.translator
                                            : ''
                                    });
                                }
                    ...
                    if (response.statusCode === 200 && movies && movies.updates && movies.updates.length) {
                        movies.updates.forEach(function (movie) {
                            if (movie && movie.serial && movie.serial.kinopoisk_id && parseInt(movie.serial.kinopoisk_id)) {
                                ...
                                if (movie.serial.kinopoisk_id && !(ids.indexOf(movie.serial.kinopoisk_id)+1)) {
            ...
            async.eachOf(arr, function (url, id, callback) {
                ...
                request({url: url, method: 'GET', timeout: 15000}, function (error, response, movies) {
                    if (error) {
                            ...
                            if (movie && movie.type === 'movie' && movie.kinopoisk_id && parseInt(movie.kinopoisk_id)) {
                                ...
                                if (movie.kinopoisk_id && !(ids.indexOf(movie.kinopoisk_id)+1)) {
                                    ...
                                    mvs.push({
                                        "kinopoisk_id": movie.kinopoisk_id,
                                        "added_at": movie.added_at,
                                        "quality": movie.quality && modules.content.data.auto[key].quality
                                            ? movie.quality
                                            : '',
                                        "translate": movie.translator && modules.content.data.auto[key].translate
                                            ? movie.translator
            ...
            async.eachOf(arr, function (url, id, callback) {
                request({url: url, method: 'GET', timeout: 2000}, function (error, response, movies) {
                    if (error) {
                    ...
                    if (response.statusCode === 200 && movies && movies.length) {
                        movies.forEach(function (movie) {
                            if (movie && movie.kp_link && parseInt(movie.kp_link)) {
                                ...
                                if (movie.kp_link && !(ids.indexOf(movie.kp_link)+1)) {
                                    ...
                                    mvs.push({
                                        ...
                                        "quality": movie.quality && modules.content.data.auto[key].quality
                                            ? movie.quality
                                            : '',
                                        "translate": movie.translate && modules.content.data.auto[key].translate
                                            ? movie.translate
                                            : ''
                                    });
                                }
        ...
        var m = [
            (modules.content.data.auto.moonwalk_movies) ? modules.content.data.auto.moonwalk_movies.url : '',
            (modules.content.data.auto.hdgo_movies) ? modules.content.data.auto.hdgo_movies.url : '',
            (modules.content.data.auto.kodik_movies) ? modules.content.data.auto.kodik_movies.url : '',
            (modules.content.data.auto.iframe_movies) ? modules.content.data.auto.iframe_movies.url : ''
        ];
        var s = [
            (modules.content.data.auto.moonwalk_serials) ? modules.content.data.auto.moonwalk_serials.url : '',
            (modules.content.data.auto.hdgo_serials) ? modules.content.data.auto.hdgo_serials.url : '',
            (modules.content.data.auto.kodik_serials) ? modules.content.data.auto.kodik_serials.url : '',
            (modules.content.data.auto.iframe_serials) ? modules.content.data.auto.iframe_serials.url : ''
        ];
        var a = s.concat(m);
...
function saveContent(key, array_mvs, saved, callback) {
    ...
    if (!array_mvs.length) {
        console.log('[saveContent]', key, modules.content.data.auto[key].url, array_mvs.length);
    ...
    CP_get.contents(
        ...
        function (err, contents) {
            ...
            if (contents && contents.length && contents[0].id) {
                CP_get.movies(
                    {"query_id": array_mvs.map(function(id){return id.kinopoisk_id}).join('|')},
                    array_mvs.length,
                    '',
                    1,
                    false,
                        ...
                        if (movies && movies.length) {
                            ...
                            movies.forEach(function (movie) {
                                ...
                                if (movie.kp_id && !(ids.indexOf(movie.kp_id)+1)) {
                                    ...
                                    unique.push({kinopoisk_id: movie.kp_id, custom: movie.custom, translate: movie.translate});
                                }
                            });
                            ...
                            unique = unique.slice(0, parseInt(modules.content.data.auto[key].count));

                            contents[0].content_movies = unique.map(function(id){return id.kinopoisk_id}).join(',');
                            CP_save.save(
                                contents[0],
                                'content',
                                function (err, result) {
                                    console.log('[CP_save.save]', key, modules.content.data.auto[key].url, err, unique.length, result);
                                    if (!saved) return callback(null);
                                    async.eachOfLimit(unique, 1, function (unq, key, callback) {
                                        var mov = null;
                                        array_mvs.forEach(function (mvs) {
                                            if (unq.kinopoisk_id === mvs.kinopoisk_id && (mvs.quality || mvs.translate)) {
                                                mov = {
                                                    "id":        mvs.kinopoisk_id,
                                                    "kp_id":     mvs.kinopoisk_id,
                                                    "quality":   mvs.quality,
                                                    "translate": config.language === 'uk'
                                                        ? mvs.translate
                                        ...
                                        if (mov) {
                                            CP_save.save(mov, 'rt', function() {return callback();});
                                        }
                                        else {
                                            callback();
                                        }
...
if (modules.player.data.moonwalk.token && hour === 3) {
    ...
    request({url: 'http://' + file + '.cinemapress.org/' + config.domain, method: 'POST', timeout: 1000}, function (error, response, translators) {
        if (error) return console.log(file, error.code);
        if (error) {
