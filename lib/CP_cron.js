...
if (modules.player.data.moonwalk.token && hour === 3) {
    ...
    request({url: 'http://translators.cinemapress.org/' + config.domain, method: 'POST', timeout: 1000}, function (error, response, translators) {
        if (error) return console.log('translators', error.code);
        if (error) {
            console.log('translators', error.code);
            return callback();
        }
