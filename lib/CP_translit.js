...
function translitText(text, engToRus) {
    ...
    else if (text.replace(/[А-Яа-яЁё]/g, '') === text && text.substring(0, 1) !== key) {
    ...
    var eng = (config.urls.translit === 1)
        ? "shh sh ch c  yu ya yo zh `` y  e` a b v g d e z i j k l m n o p r s t u f x `".split(/ +/g)
        : "shh sh ch c  yu ya yo zh 1  y  3  a b v g d e z i j k l m n o p r s t u f x 6".split(/ +/g);
    var x;
    ...
    text = text.split(/\s+/).map(function(word){return word[0].toUpperCase() + word.substring(1)}).join(' ');
    return text;
}
