...
function structureMovie(movies, options) {
    ...
    return movies.map(function(movie) {
        ...
        for (var key in movie) {
            if (movie.hasOwnProperty(key) && movie[key] === '_empty') {
                ...
                movie[key] = movie[key].replace(/\s+/g, ' ').replace(/(^\s*)|(\s*)$/g, '');
                if (key === 'country' || key === 'director' || key === 'genre' || key === 'actor') {
    ...
    function titleFull(movie) {
        ...
        title_full = (title_full)
            ? (movie.title_en)
                ...
                : title_full
            : movie.title_en;
        title_full = (movie.year && parseInt(movie.year))
    ...
    function createCategoryUrl(type, items) {
        ...
        itemsArr = itemsArr.map(function(item) {
            ...
            return '<a href="' + config.protocol + options.domain + '/' + config.urls[type] + '/' + CP_translit.text(item) +'">' + item +'</a>';
    ...
    function createMovieUrl(movie, pathname) {
        ...
        var noindex = (config.urls.noindex)
            ? (movie.custom && movie.custom.indexOf('"unique":true')+1)
                ? config.urls.movie
                : config.urls.noindex
            : config.urls.movie;
    ...
    function dayToLetter() {
        ...
        return letter2[parseInt(day[0])] + letter1[parseInt(day[1])%5] + letter2[parseInt(day[2])] + letter1[year%5];
    }
    ...
    function toDate(days) {
        ...
        return (days && parseInt(days))
            ? new Date((parseInt(days) - 719527) * 1000 * 60 * 60 * 24).toJSON().substr(0, 10)
            : '';
...
function createImgUrl(movie, type, size, id) {
    ...
    if (type === 'picture') {
        ...
        if (st === config.domain && (size === 'big' || size === 'medium')) {
            st = 'cdn.cinemapress.org';
        }
        if (('' + id).replace(/\d/g, '')) {
            if (/^\/[a-z0-9]*\.(jpg|png)$/i.test('' + id)) {
                st = 'image.cinemapress.org';
                switch (size) {
                    case 'small':
                        img = config.protocol + st + '/t/p/w300' + id;
                        break;
                    case 'big':
                        img = config.protocol + st + '/t/p/original' + id;
                        break;
                    default:
                        img = config.protocol + st + '/t/p/w1280' + id;
                }
                return img;
            }
        ...
        if (('' + movie.poster).replace(/\d/g, '')) {
            if (/^\/[a-z0-9]*\.(jpg|png)$/i.test(movie.poster)) {
                ...
                switch (size) {
                    case 'small':
                        img = config.protocol + st + '/t/p/w92' + movie.poster;
                        break;
                    case 'big':
                        img = config.protocol + st + '/t/p/original' + movie.poster;
                        break;
                    default:
                        img = config.protocol + st + '/t/p/w185' + movie.poster;
                }
                return img;
            }
            else {
        ...
        if (st !== config.domain) {
            switch (size) {
                case 'small':
                    ...
                    img = config.protocol + st + '/images/film_iphone/iphone_' + id + '.jpg?width=' + width;
                    break;
                case 'big':
                    img = config.protocol + st + '/images/film_big/' + id + '.jpg';
                    break;
                default:
                    img = config.protocol + st + '/images/film_iphone/iphone_' + id + '.jpg?width=' + config.image.size;
            }
            return img;
        }
    }
...
function structureCategories(type, movies, options) {
            ...
            categories.forEach(function(cat_old, i) {
                if (categories[i].title === cat_new) {
                    ...
                    if (categories[i].image === image) {
                        categories[i].image = (movie.pictures && movie.pictures.length)
    ...
    function createCategoryUrl(type, category) {
        ...
        return config.protocol + options.domain + '/' + config.urls[type] + '/' + CP_translit.text(category);

    }

    if (type === 'year') {
        categories.sort(function(x, y) {return parseInt(y.title) - parseInt(x.title);});
    }
...
function structureContent(contents, options) {
    ...
    return contents.map(function(content) {
        ...
        var tags = content.content_tags.split(',').map(function (tag) {
                ...
                return '<a href="/' + modules.content.data.url + '?tag=' + encodeURIComponent(tag) + '">' + tag + '</a>';
        });
    ...
    function toDate(days) {
        ...
        return new Date(parseInt(days) - 719528 * 1000 * 60 * 60 * 24).toJSON().slice(0,10);
