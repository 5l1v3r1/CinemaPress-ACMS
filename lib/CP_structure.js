...
function structureMovie(movies, options) {
    ...
    function changeDescription(movie, short) {
        ...
        if (short) {
            var match = reg.exec(movie.description);
            if (match && match.length) {
                text = match[2];
            }
            else if (movie.description_short) {
    ...
    function createMovieUrl(movie, pathname) {
        ...
        return (typeof pathname !== 'undefined')
            ? '/' + config.urls.movie + '/' + url
            : config.protocol + options.domain + '/' + config.urls.movie + '/' + url;
...
function structureContent(contents, options) {
    ...
    return contents.map(function(content) {
        ...
        content = {
            ...
            "image"       : content.content_image || options.content_image || '',
            "tags"        : tags.join(', '),
