...
function structureMovie(movies, options) {
    ...
    return movies.map(function(movie) {
        ...
        movie = {
            ...
            "description_short" : changeDescription(movie.description, true),
            "description"       : changeDescription(movie.description),
            "year"              : movie.year,
    ...
    function createMovieUrl(movie) {
        ...
        return 'http://' + options.domain + '/' + config.urls.movie + '/' + url;
    ...
    function createImages(movie) {
        ...
        images.poster = (('' + movie.poster).replace(/[0-9]/gi, ''))
            ? movie.poster
            : config.protocol + st + '/images/film_iphone/iphone360_' + movie.kp_id + '.jpg';
        images.poster_min  = config.protocol + st + '/images/sm_film/' + movie.kp_id + '.jpg';

        if (st == config.domain) {
            images.poster = createPosterUrl(movie, 'medium');
            images.poster_min = createPosterUrl(movie, 'small');
            st = 'st.kp.yandex.net';
        }
    ...
    function createPosterUrl(movie, type) {
        ...
        return '/images/poster/' + type + '/' + url + '.jpg';
