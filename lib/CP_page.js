...
function pageMovie(result, type, options, callback) {
    ...
    if (options.debug) {
        options.debug.detail.push({"type": "seo", "duration": (new Date() - options.debug.duration.current) + 'ms'});
    ...
    if (options.debug) {
        options.debug.detail.push({"type": "meta", "duration": (new Date() - options.debug.duration.current) + 'ms'});
        options.debug.duration.current = new Date();
    }
    ...
    if (modules.player.status || type === 'trailer' || type === 'picture') {
        ...
        if (options.debug) {
            options.debug.detail.push({"type": "player", "duration": (new Date() - options.debug.duration.current) + 'ms'});
            options.debug.duration.current = new Date();
        }
    }
    if (modules.viewed.status) {
        ...
        if (options.debug) {
            options.debug.detail.push({"type": "viewed", "duration": (new Date() - options.debug.duration.current) + 'ms'});
            options.debug.duration.current = new Date();
        }
    }
    if (modules.continue.status) {
        ...
        if (options.debug) {
            options.debug.detail.push({"type": "continue", "duration": (new Date() - options.debug.duration.current) + 'ms'});
            options.debug.duration.current = new Date();
        }
    }
    if (modules.social.status) {
        ...
        if (options.debug) {
            options.debug.detail.push({"type": "social", "duration": (new Date() - options.debug.duration.current) + 'ms'});
            options.debug.duration.current = new Date();
        }
    }
    ...
    if (modules.mobile.status) {
        ...
        if (options.debug) {
            options.debug.detail.push({"type": "mobile", "duration": (new Date() - options.debug.duration.current) + 'ms'});
            options.debug.duration.current = new Date();
        }
    }
    if (modules.episode.status) {
        if (!page.title) {
            ...
            page.h1          = optimalLength(CP_text.formatting(modules.episode.data.h1, movie));
            page.description = optimalLength(page.seo);
        }
    ...
    if (modules.schema.status) {
        ...
        if (options.debug) {
            options.debug.detail.push({"type": "schema", "duration": (new Date() - options.debug.duration.current) + 'ms'});
            options.debug.duration.current = new Date();
        }
    }
    if (modules.adv.status) {
        ...
        if (options.debug) {
            options.debug.detail.push({"type": "adv", "duration": (new Date() - options.debug.duration.current) + 'ms'});
            options.debug.duration.current = new Date();
        }
    }
    if (config.cache.p2p) {
        ...
        if (options.debug) {
            options.debug.detail.push({"type": "p2p", "duration": (new Date() - options.debug.duration.current) + 'ms'});
            options.debug.duration.current = new Date();
        }
    }
...
function pageCategory(result, query, sorting, num, options, callback) {
    ...
    if (!/\s/g.test(page.title)) {
        page.description = page.description.charAt(0).toUpperCase() + page.description.slice(1);
    }
