...
function pageMovie(type, movie, movies, options) {
    ...
    page.domain   = options.domain;
    page.email    = config.email;
    page.url      = (type == 'movie') ? movie.url : movie.url + '/' + config.urls.movies[type];
    page.urls     = formatUrls(config.urls);
    page.codes    = formatCodes(config.codes);
 ...
 * Adding basic information on the movie page:
 * protocol, domain, email, urls, codes, seo, title,
 * description, url, sorting, pagination,
 * head, footer.
 *
 * Data from modules:
 * social.
...
function pageCategory(query, sorting, num, all, movies, options) {
...
function pageCollection(url, sorting, num, all, movies, options) {
...
function pageCollections(options) {
...
function pageCategories(category, options) {
